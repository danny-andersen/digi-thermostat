...
class AdjSetTimeConstants(Structure):
    ...
    ("extAdjustment", c_short)] #in tenths
...
class SchedByElem(LittleEndianStructure):
    _fields_ = [
        ...
        ("temp", c_short)] #Set temperature tenths of C, 180 = 18.0C
...
class StationContext():
    ...
    def getSavedContext(self) -> dict:
        ...
        if (path.exists(stationFile)) :
            try:
                ...
                print(f"Failed to load json data from station context file {stationFile}")            
        return loadStatus
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.currentBoilerStatus == other.currentBoilerStatus \
...
def getMessage():
    ...
    if not path.exists(SCHEDULE_FILE):
        sc.setSchedTime = 0
    if sc.tempMotdTime and (datetime.now().timestamp() - sc.tempMotdTime > TEMP_MOTD_EXPIRY_SECS):
        #Check if temp motd has expired
        sc.tempMotdTime = 0
        sc.motdTime = 0 #Resend any motd
    response:Response = getNoMessage()
    if len(sc.scheduleMsgs) == 0 and path.exists(SCHEDULE_FILE) and stat(SCHEDULE_FILE).st_mtime > sc.setSchedTime:
    ...
    elif (not sc.tempMotdTime and path.exists(MOTD_FILE) and stat(MOTD_FILE).st_mtime > sc.motdTime):
...
def createMotd(str, motdExpiry = TEMP_MOTD_EXPIRY_SECS):
    ...
    class Motd(Structure):
        _fields_ = [
            ...
            ("motdStr", c_char * motdLen)] #Message of the day, with a minimum of one character
    motd = Motd()
    ...
    if (path.exists(SET_TEMP_FILE)) :
        with open(SET_TEMP_FILE, "r", encoding="utf-8") as f:
            try:
                ...
                msgBytes = getMessageEnvelope(SET_TEMP_MSG, bytearray(tempMsg), sizeof(Temp))
                response = Response(response=msgBytes, mimetype='application/octet-stream')
                gotTemp = True
...
def getNextScheduleMsg(sc: StationContext):
        ...
        if schedStr == None:
            ...
            msgBytes = getMessageEnvelope(SCHEDULE_MSG, bytearray(schedMsg), sizeof(SchedByElem))
